...
- **`GET /produtos/:codigo/derivacoes`** — lista as outras derivações do mesmo produto pai: `[{ codigo, nome, ativo }]`. Aceita qualquer derivação (não precisa já saber o código do pai — resolve por dentro, igual `GET /produtos/:codigo` já faz pro estoque unificado). Chamado de "derivação" de propósito, não "cor": nem todo produto varia por cor (pode ser modelo, tamanho etc.), então o nome genérico da própria Magazord é o certo aqui.
