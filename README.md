...
Dois endpoints pra montar a página/seletor de variação de um produto. **Públicos** (sem `Authorization`) — o player da live é anônimo e mostra as cores/fotos ali; ficam antes do `requireUser` no router, igual `GET /lives/:id/audiencia`. Não expõem nada sensível (é a mesma info da página pública da loja). Ambos aceitam **qualquer** código de derivação e resolvem o produto pai por dentro. Tudo sai do **feed da vitrine da Magazord** (`/v2/site/frontend/produto/<loja>/<codigo>`) — a mesma fonte que a própria página de produto da loja usa pra renderizar; nada é heurística sobre string. O serviço tem um **cache curto (1 min)** por `codigo` pra não repetir as chamadas à Magazord quando muita gente na mesma live abre o mesmo produto.

> `GET /produtos/:codigo` e `GET /produtos/buscar` **continuam autenticados** (uso do painel) — o `:codigo` faz upsert em `produto_caracteristicas` e usa o desconto de Pix do usuário; `/buscar` é autocomplete do admin.
...
- **`GET /lives/:id/produtos`** — **pública, sem token**. Produtos ativos da live (`live_products` onde `ativo = true`), ordenados por `ordem` — a lista que o player mostra na tela. Antes o front lia `live_products` direto do Supabase (RLS); a **leitura inicial** passou pra cá pra ser sempre pelo backend (service role, shape controlado). A assinatura **Realtime** dos updates ao vivo (produto adicionado/removido/destacado no meio da live) continua no canal `postgres_changes` do Supabase — o backend não faz push, e as escritas já passam todas por `/live-products` (autenticado). Live inexistente → `200` `[]`.
...
### Cupons de desconto
Cria o cupom de verdade na Magazord (funciona no checkout real da loja) e guarda a referência aqui (a Magazord não sabe o que é uma "live"). Leitura de `cupons` é direta no Supabase (RLS pública), igual `lives` — essas rotas são só pra escrita. (`live_products` e `produto_caracteristicas` já não: leitura via `GET /lives/:id/produtos` e `GET /produtos/:codigo/caracteristicas`.)
...
Guardado numa tabela própria (`empresa_temas`), separada de `empresa_configuracoes`, porque agora existem **2 conjuntos completos de cores por empresa** — um pro modo claro e um pro modo escuro — em vez de 1 só. Leitura pública (RLS) é direta no Supabase pra quem já sabe o `empresa_id` (o player); o painel usa a rota `/me` abaixo pelo mesmo motivo da seção acima. Duas linhas (`modo: 'claro'` e `modo: 'escuro'`) são criadas automaticamente com valores padrão quando a empresa é criada — a paleta padrão da plataforma (`services/temas.js`/`.ts`, `TEMA_PADRAO_CLARO`/`TEMA_PADRAO_ESCURO`): verde-escuro `#546B41` (primária), verde-sálvia `#99AD7A` (acento), bege `#DCCCAC` (secundária) e creme `#FFF8EC` (base clara) no modo claro; uma variante escura coerente com as mesmas cores no modo escuro (não é cópia do claro).
- **`GET /empresa-temas/me`** — resolve a empresa do usuário logado sozinho e devolve `{ empresa_id, claro, escuro }` (os 2 modos de uma vez). Se por algum motivo faltar linha pra essa empresa (não deveria acontecer, já que a criação sempre insere as 2 — cobre dado legado/incompleto), devolve a paleta padrão no lugar em vez de erro. `404` só se o usuário não pertencer a exatamente 1 empresa.
...
- **`GET /metricas?empresa_id=<opcional>`** — visão cross-empresa em tempo real: lives `ao_vivo` agora (com audiência atual, cruzando com o servidor de live) + saúde de cada instância do servidor de live (`servidores: [{ instancia, cpu, mem, conns, uptime_s, recv_bytes, send_bytes }, ...]` — array, 1 item por instância, já que o servidor de live roda em sharding/múltiplos processos SRS, ver README do `sereniia-org/test-live-server`) + um resumo `por_empresa`. Sem `empresa_id`, mostra tudo; com ele, filtra pra uma empresa só (é um filtro pro super-admin fazer drill-down, não um controle de acesso — só super-admin chama isso de qualquer forma).
