...
// captura (metricas.ts, POST /metricas/capturar) já foi salvando em
...
export async function historicoAudienciaLive(liveId: string) {
  ...
  const valores = ((data as any[]) ?? []).map((p) => p.espectadores ?? 0);
  return {
    pico_espectadores: valores.length ? Math.max(...valores) : null,
    media_espectadores: valores.length ? Number((valores.reduce((a: number, b: number) => a + b, 0) / valores.length).toFixed(1)) : null,
    pontos: data ?? [],
  };
}
